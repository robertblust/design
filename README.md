...
A tag may also carry `data-questions`, a same-origin path to the site's own already-published model — blust.ch's `/model.json`, companygraph.io's `/company.json` — the same file `card.js` and `stage.js` already read; once the panel is shown on an empty conversation, the widget takes the `name` of every entity whose `type` is `question` as a title and offers three of them, picked at random, as chips to start from. That read is same-origin and reaches no service the family runs, so the promise above — that nothing leaves for the chat host before a visitor presses send — holds for it exactly as it holds for the button and the notice; a tag without `data-questions` offers no chips and asks nothing.
