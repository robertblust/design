...
A site takes the chat by naming `chat` in `design.config.json`'s groups, as it does `stage`; `npm run design` then copies the two files, `chat.js` and `chat.css`, into the site. A page that offers the chat loads one more script, `chat.js`, with the endpoint and the model page on its own tag, `<script src="chat.js" data-chat="https://chat.blust.ch/chat" data-model="/model/" defer>`, and links `chat.css` beside its stylesheet; a tag without `data-chat` shows nothing. The button, the panel and every sentence the widget writes are the package's, in both languages; a site adds nothing but the tag. The line under an answer is the receipt for the rule that every claim comes from a tool's answer: it opens with the page's own icon, read from its `<link rel="icon">`, so a site that wants one there declares one in its head and adds nothing to the tag, and after each cited title stands GitHub's mark to the file at the commit the host serves; a page that declares no icon keeps the words "From the model". Where the chat host names the moment a limit lifts, the widget's refusal ends with it, in the page's language and the visitor's local time; a host that names none gets the plain sentence.

A tag may also carry `data-questions`, a same-origin path to the site's own already-published model — blust.ch's `/model.json`, companygraph.io's `/company.json` — a file the site already publishes; once the panel is shown on an empty conversation, the widget takes the `name` of every entity whose `type` is `question` as a title and offers three of them, picked at random, as chips to start from. Every finished answer offers three more the same way, leaving out any title the conversation has already asked, so a visitor always has a next question to hand; a conversation at its twenty-message limit offers none. That read is same-origin and reaches no service the family runs, so the promise above — that nothing leaves for the chat host before a visitor presses send — holds for it exactly as it holds for the button and the notice; a tag without `data-questions` offers no chips and asks nothing.
...
Beside the whole files this package copies into a site, some of its blocks live *inside* a page: a fence is a pair of comment markers the page already carries — `design tokens`, `header contract`, `title contract`, `lines`, `stage contract`, `prose reset`, `language`, `theme boot`, `theme`, `nav fit`, `prose footer`, `deck transport`, `deck lockup`, `deck fit`, `deck runtime`, `team`, `surfaces`, `surfaces lineage`, `principles`, `index`, `link` and `model card` — and the package owns everything between and including them, prose, version and CSS alike. `lines` is where a line's color keeps one meaning across the family: the title note's caveat in `--c-flag`, once per page, and a section's conclusion in `--c-sum`, a green beside the blue confidence ramp, at most once per section. A risk has no line yet, because `--warn` and `--c-flag` cannot be told apart by a reader with red-green color blindness, and a risk line waits for a second carrier. That is the whole reason a fence is not just another synced file: a synced file is copied whole, but a fenced block sits in the middle of a page the tool never fully owns, so the markers are what tell it exactly where its part starts and stops.
...
Five names are not fences: `tokens.css`, `page.css`, `page.js`, `deck.css` and `deck.js` are the same blocks `blockFor` already reads, written as five whole files a site copies exactly as it copies `chat.css`, rather than into a gap between two markers. `lib/assemble.mjs` reads a file's blocks in a fixed order, drops each block's own marker comment — a file carries no gap in a page for a marker to bound — and the two spaces a block carried inside a page's `<style>` or `<script>`, and opens with a comment naming the blocks it was assembled from, each with its version from `versions.json`, and saying that editing the file in a site does nothing, because the next `npm run design` overwrites it. The comment names no release, so a file's bytes change exactly when a block in it changes: a release that touches only the checks, the exporters or this README leaves the five files and every share-card stamp as they were, and a site's re-pin is its pin and lockfile alone. The release a site is on is read from its pin. `tokenVersion` reads the tokens version from a linked `tokens.css`'s comment and holds it to the same number it holds a fenced page's `design tokens` marker to. `deck.css` needs no tokens of its own beyond the shared ones: what a deck page used to add after the `design tokens` fence, once the brace it left open is put back, turned out to be nothing at all, so `tokens.css` alone, always closed, is what a deck links too.
...
The cascade order changes, not only which rules exist. A fenced page interleaved its own CSS with each fenced block, in whatever order the page happened to write the two; `page.css` and `deck.css` sit in one `<link>` before any of the page's own `<style>`, so every shared rule now comes first and the page's own rules come after all of it. A page-own rule that used to lose to a fenced rule at equal specificity — because the page wrote it earlier in the cascade — can now win, since it is later in every page that takes the files. A site moving should look at its own rules on the selectors the shared blocks also touch: `*`, `body`, `a`, `.mono`, `.shell`, `nav`, `nav a`, `.seg`, `footer`, `footer a`, `.title h1`, `.tagline`, and the classes the model pages own (`.legend`, `.card`, `.row`, `.board` among them, per the paragraph above).

A German visitor can meet one frame of English. The fenced form called `applyLang(lang)` inline, in the page's own script, before the browser had painted anything; `page.js` makes that same call, but only once it has arrived, and a `<script src>` — deferred so it runs after parsing, like the other two links — is a request the page's own inline script never was. Between first paint and that request landing, a page whose markup is English-first (see "Crawlers", and the `sourceLang` check in `verify/pages.mjs`) shows English, for however long the file takes to arrive. This is the same reason `theme boot` stays a fence rather than becoming a file: a wrong theme is seen, instantly, as a flash of the wrong background; a wrong language is read, and a reader is already partway into the first sentence before a language they were not expecting registers. The two failures are the same shape and only one of them stayed a fence — accepted here because reading the wrong language for a moment costs less than seeing the wrong theme for one.
...
Each writer takes `(data, { check, root })`, and `writeTeam` also takes `order`, the names of the processes to draw first, in that order, because core gives a process no rank and the order a page argues in is the site's to choose; the rest follow in the artifact's order, and a name the model does not hold throws. On every board the seat that owns the process, as its file names it, is the first row. A role names nobody and any number of profiles may hold it, so a row carries every holder: a seat held by several draws two marks, one behind the other, with a person's in front when a person is among them, and its label names them all. On the rail, profiles that hold exactly the same seats on a board are one entry that shares its count, so the rail's counts add up to the board's; a profile whose seats differ keeps its own entry and counts every seat it sits in. `root` is the site's own root, required rather than guessed from where the package sits, and a writer called without it throws the reason why; `check` reports the files it would change without writing them, as `design sync --check` does for the whole page.

`team`, `surfaces` and `principles` are each one page's own CSS and nothing else. `link` is the family's one link style, blue with a line only under the pointer, and rides in both `page.css` and `deck.css` so a page writes no link rule of its own. `index` is the list an index page draws under its title, on /talks/ and /blog/ alike, and is scoped to `.index` because it rides in `page.css`. `surfaces lineage` and `model card` are behavior instead. `surfaces lineage` draws the lineage and holds Surfaces's card glue together, because the two share state: the card in the panel is whichever surface is chosen, and choosing one redraws the wires too. `model card` is Team's card glue — the rows behind a board and the one Open-all over them. Both need a value the page declares above them in the same script, `STAGE_PAGE`, ending in `/`: the page that draws the model on the stage, where the links in a card go, a seat's on Team and a surface's on Surfaces; `model card` also needs `MODEL_CARD`, the name `card.js` reports a failed read under, declared the same way.

The suite's half is `verify/model-pages`. It exports `MODEL_PAGE_CHECKS`, which a site spreads into its checks beside `STAGE_CHECKS`, and a page opts in with `board: true` on Team or `lineage: true` on Surfaces. Neither check carries a name from any model: `board` reads each board's phase headings, rows and gates from the artifact the page names, through the same helpers `writeTeam` uses, and `lineage` reads each surface's maker the same way. Both fetch the page `STAGE_PAGE` names and fail unless it draws the same artifact, because a card's links are written when it opens and nothing else could see a wrong target.
