...
// has to notice.
// Tools is first, by the owner's decision: a reader who came to set the thing up wants that
// before anything a site says about it, and a site without a Tools page loses nothing, since
...
// published there.
test("navOrder's rule puts Tools first, then Team and Principles", () => {
  const src = pageChecks(OPTS).navOrder.toString();
  const m = /const ORDER = \[([^\]]+)\]/.exec(src);
  assert.ok(m, "navOrder has no ORDER list");
  const order = m[1].split(",").map((s) => s.trim().replace(/"/g, "")).filter(Boolean);
  assert.equal(order[0], "Tools");
