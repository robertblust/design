...
export const FENCES = {
  ...
  // The title block every prose page opens with, and the first fence added because copies
  // had already drifted rather than because they had not: four of the fourteen pages had a
  // different headline treatment entirely. The landing pages are deliberately out — see the
  // block's own header for why three heroes stay three.
  "title contract": {
    key: "title", source: "blocks/title.css", version: versions.title,
    variants: null, closes: null,
  },
  // The lines a prose page draws beside a paragraph: the title note's caveat in --c-flag and a
  // section's conclusion in --c-sum. One place for what a line's color means.
  "lines": {
    key: "lines", source: "blocks/lines.css", version: versions.lines,
    variants: null, closes: null,
  },
  ...
  // The family's one link style, in page.css after the reset and in deck.css before the
  // lockup, so a prose page and a deck draw a link in text the same way. No variants.
  "link": {
    key: "link", source: "blocks/link.css", version: versions.link,
    variants: null, closes: null,
  },
  // Four rules at the top of every prose page, between the tokens fence and the page's own
  ...
  // Prose pages only. A deck has no `.bar` — its chrome is the transport — and the block
  // returns on the first line without one, but a fence a page cannot use is a fence that page
  // should not carry.
  "nav fit": {
    key: "navFit", source: "blocks/nav-fit.js", version: versions.navFit,
    variants: ["page"], closes: null,
  },
  ...
  // The rest are the three model pages' own code, which moved here with their renderers: a page
  // whose markup is written by this package and whose styling and behavior are its own copy is a
  // page whose two halves drift, and a class the renderer adds lands on every site unstyled. No
  // variants: each is one page's, and no site's copy differs. A class several pages share
  // (`.lbl`, `.nm`, `.note`, `.mono`, `.lede`, `.derived`) is not in any of them; it stays with
  // the page, which is where every other page that uses it keeps it too.
  //
  // The board /team/ draws: head rail, grid, drawer, legend, phases, and the process heading
  // above each board when a model holds more than one process.
  "team": {
    key: "team", source: "blocks/team.css", version: versions.team,
    variants: null, closes: null,
  },
  // The lineage /surfaces/ draws: the model, its makers, their surfaces and the wires between.
  "surfaces": {
    key: "surfaces", source: "blocks/surfaces.css", version: versions.surfaces,
    variants: null, closes: null,
  },
  // The lineage's script, with the Surfaces card glue inside it because the two share state:
  // the card shown is the surface chosen. Its links go to a `STAGE_PAGE` the page declares
  // above the fence — see the block's header.
  "surfaces lineage": {
    key: "lineage", source: "blocks/surfaces.js", version: versions.lineage,
    variants: null, closes: null,
  },
  // The values /principles/ lists. Its title block is the `title contract` fence's.
  "principles": {
    key: "principles", source: "blocks/principles.css", version: versions.principles,
    variants: null, closes: null,
  },
  // The card glue for a board of seats, one Open-all per board. Like `deck runtime` it has a
  // contract with the page that the markers cannot see: the page declares `STAGE_PAGE` and
  // `MODEL_CARD` above the fence — see the block's header.
  "model card": {
    key: "modelCard", source: "blocks/model-card.js", version: versions.modelCard,
    variants: null, closes: null,
  },
};
