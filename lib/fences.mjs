...
export const FENCES = {
  ...
  // the address already carries. v2 took it from the site as a `langKey` parameter, on the
  // argument that a storage key is a site's own promise; the family decided it is one promise
  // across the three, and a promise the package makes is one no site can quietly make
  // differently. No fence declares a parameter today; the mechanism stays for one that does.
  "language": {
  ...
  // blocks/lang.js byte for byte, so this fence's pass and the nested fence's own agree. They
  // used to share a `langKey` parameter for that; the key is the family's now and both carry
  // the literal.
  "deck runtime": {
