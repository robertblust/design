...
export const FENCES = {
  ...
  "deck lockup": {
    ...
    // own value (a mark's colour, its pixel size, a selector's breadth) and belongs entirely
  ...
  //
  // Declares `langKey` for the same reason "language" does, not because this fence needs a
  // second, independent value: the nested `language` fence embedded inside this block's own
  // source shares the same `{{langKey}}` slot, and both fences' passes must fill it with the
  // same site value or the two disagree forever. v1 shipped that slot pre-substituted with
  // blust.ch's own key — correct for one site, wrong for the other two, and with no fixed
  // point: the nested fence's own pass would keep correcting the visible bytes to the site's
  // real key while this fence kept re-emitting blust.ch's, a permanent `design:check` failure.
  "deck runtime": {
    key: "runtime", source: "blocks/deck-runtime.js", version: versions.runtime,
    variants: null, closes: null, params: ["langKey"],
  },
};
