...
export function readConfig(siteRoot) {
  ...
  // The config names the groups a site takes and nothing else; no fence reads it. A key it
  // does not name is an error rather than something to skip, because a value nothing reads is
  // a value someone still believes in.
