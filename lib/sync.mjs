...
export function readConfig(siteRoot) {
  ...
  // The config names the groups a site takes and nothing else. It used to carry the storage
  // keys as parameters the fences read by name; those are the family's now and no fence reads
  // the config at all. A key left behind is an error rather than something to skip, because a
  // value nothing reads is a value someone still believes in.
  for (const key of Object.keys(parsed))
    if (key !== "groups")
      throw new Error(
        `${file} carries "${key}", which nothing reads — the config names "groups" and ` +
        `nothing else; remove it.`);
