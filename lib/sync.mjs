...
export function readConfig(siteRoot) {
  ...
  // The config names the groups a site takes and, for the two variants the "files" group
  // needs, the words that choose them — "footer" and "lockup" — and nothing else; no fence
  // reads it. A key it does not name is an error rather than something to skip, because a
  // value nothing reads is a value someone still believes in.
  for (const key of Object.keys(parsed))
    if (key !== "groups" && key !== "footer" && key !== "lockup")
      throw new Error(
        `${file} carries "${key}", which nothing reads — the config names "groups", ` +
        `"footer" and "lockup" and nothing else; remove it.`);

  // Built up rather than spread whole: a site that never mentions "footer" or "lockup" gets a
  // plain { groups } back, unchanged from every release before this one, rather than the same
  // object with two keys added at `undefined` — a shape a strict deepEqual sees and a site
  // that only ever reads `.groups` never would.
  const config = { groups };
  if ("footer" in parsed) config.footer = parsed.footer;
  if ("lockup" in parsed) config.lockup = parsed.lockup;
  return config;
}

// Compare without writing. `state` is the entire vocabulary the CLI reports in.
//
// Every other group is a [from, to] pair copied byte for byte from the package. "files" has no
// "from": there is nothing on disk to copy, only a name assemble() turns into bytes from the
// config a site declared. Its entries carry `kind: "assemble"` so applySync knows to call
// assemble() again rather than read a file that does not exist.
export function planSync(siteRoot, config) {
  const entries = [];
  for (const group of config.groups) {
    if (group === "files") {
      for (const name of GROUPS.files) {
        const want = Buffer.from(assemble(name, config), "utf8");
        const dest = path.join(siteRoot, name);
        let state;
        if (!fs.existsSync(dest)) state = "missing";
        else state = fs.readFileSync(dest).equals(want) ? "same" : "differs";
        entries.push({ from: name, to: name, state, kind: "assemble" });
      }
      continue;
    }
...
// status` stays quiet. `config` is only ever read for an entry `planSync` marked
// `kind: "assemble"`; every other entry still needs nothing but the bytes already on disk under
// PKG, exactly as before this group existed.
export function applySync(siteRoot, entries, config = {}) {
  const written = [];
  for (const e of entries) {
    if (e.state === "same") continue;
    const dest = path.join(siteRoot, e.to);
    fs.mkdirSync(path.dirname(dest), { recursive: true });
    const bytes = e.kind === "assemble"
      ? Buffer.from(assemble(e.from, config), "utf8")
      : fs.readFileSync(path.join(PKG, e.from));
...
export function planFences(siteRoot) {
  ...
  for (const page of findPages(siteRoot)) {
    ...
    for (const fence of FENCE_NAMES) {
      ...
      // function's job, because this is where the fence's declared `variants` list lives.
      // `variant` is never `null` on this line: `found` is only truthy when `open()` matched
      // the whole opening line, and its pattern requires a non-empty third segment, so a
      // matched fence always carries a real word here. A marker with NO third segment at all
      // (today's hand-written "deck footer · v1" on all three sites, for one) does not match
      // `open()` as this fence's opening line in the first place — `findFence` returns `null`
      // for it, and the `if (!found) continue;` above already skipped it, silently: no entry,
      // no error, `design:check` reports nothing wrong. That gap is pre-existing and this
      // function does not close it; this check only catches a fence that DOES carry a word,
      // and the word is not one this fence declares.
      if (FENCES[fence].variants && !FENCES[fence].variants.includes(variant))
        throw new FenceError(
          `${page}: the "${fence}" fence's opening line names "${variant}", but this fence ` +
          `only knows ${FENCES[fence].variants.join(", ")} — fix the variant word on the ` +
      ...
      // or leaves it open for the page to close later, after its own tokens. A deck mislabeled
      ...
      // stays because it catches a mislabeled fence *before* any sync has ever run, purely from
      ...
      // \n-joined text. Normalizing only for this equality check keeps that asymmetry — the
      ...
      if (FENCES[fence].variants) {
        const candidates = Object.fromEntries(
          FENCES[fence].variants.map((v) => [v, v === variant ? want : blockFor(fence, v)]));
...
export function applyFences(siteRoot, entries) {
  ...
  for (const [page, list] of byPage) {
    ...
    for (const e of list)
      text = replaceFence(text, e.fence, blockFor(e.fence, e.variant));
